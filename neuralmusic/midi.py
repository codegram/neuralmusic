...
__all__ = ['Triplet', 'parse_single_note', 'parse_chord', 'parse_rest', 'transpose_to_C', 'parse_part',
           'parse_midi_file', 'row_to_triplets', 'triplet_to_note', 'triplets_to_stream', 'write_midi']
...
def _parse_duration(d: duration.Duration) -> Optional[str]:
    ...
    name = d.fullName
    if dur > 0.0 and dur <= 32.0 and name is not "Inexpressible":
        return name
    else:
        return None


def parse_single_note(note: note.Note) -> Optional[Triplet]:
...
def parse_chord(chord: chord.Chord) -> Optional[Triplet]:
...
def parse_rest(rest: note.Rest) -> Optional[Triplet]:
...
def transpose_to_C(s: stream.Stream) -> (stream.Stream, str):
...
def parse_part(elements, mode) -> Optional[dict]:
    d = {
        "notes": [],
        "instrument": "unknown",
        "bpm": 120,
...
def parse_midi_file(file: str) -> Optional[pd.DataFrame]:
    ...
    """
    try:
        midi, mode = transpose_to_C(converter.parse(file))
        parts = instrument.partitionByInstrument(midi)
        rows = []
        if parts:
            for part in parts:
                parsed = parse_part(part.recurse(), mode)
                if parsed is not None:
                    rows.append(parsed)
        else:
            parsed = parse_part(midi.flat.notes, mode)
            if parsed is not None:
                rows.append(parsed)
